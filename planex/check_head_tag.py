...
def parse_args_or_exit(argv=None):
    ...
    parser.add_argument('--tag', default=None, help="New release version")
    parser.add_argument('--description', default=None, help="Release note")
    ...
    for _, url in sources:
       if "github.com" in url.netloc:
		...
		sys.exit(1)
            ...
            head = requests.get("https://api.github.com/repos/%s/%s/git/refs/heads/master" % (owner, repo), headers=headers)

            if head.json()['object']['sha'] == latest_tag['commit']['sha']:
                print "%s: tag %s is even with HEAD (%s)" % (spec.name(), latest_tag['name'], head.json()['object']['sha'])
                sys.exit(0)

            print "%s: HEAD (%s) is newer than the latest tag %s (%s)" % (spec.name(), head.json()['object']['sha'], latest_tag['name'], latest_tag['commit']['sha'])
            ...
            new_tag = args.tag
            if new_tag is None:
                new_tag = raw_input("New tag (current tag %s): " % latest_tag['name'])

            relnote = args.description
            if relnote is None:
                relnote = raw_input("Release note text: ") 

            req = { "tag_name": new_tag,
            ...
            res = requests.post("https://api.github.com/repos/%s/%s/releases" % (owner, repo), headers=headers, json=req)
            print res.url
            print req
            print res
            print res.json()
